...
#
# Let f(x) be the probability that Alice wins the game when we already have x points.
# When she has between K and N points, she stops drawing and wins. If she has more than N points, then she loses.
#
...
class Solution(object):
    def new21Game(self, N, K, W):
        if N >= K-1 + W: # largest points can draw
            return 1.0

        dp = [0.0] * (K + W)
        # dp[x] = the probability Alice wins when she has x points
        for k in range(K, min(N, K-1+W) + 1):
            dp[k] = 1.0

        S = min(N - (K-1), W) # number of 1.0 set above
    ...
    def new21Game_TLE(self, N: int, K: int, W: int) -> float:
        if N >= K-1 + W: # largest points can draw
            return 1.0

        dp = [0.0] * (K+W) # most points need to consider
        dp[0] = 1.0
    ...
    # Wrong solution because Alice stops when ponits >= K, she won't continue to see all the probability to reach [K, N] points
    def new21Game_wrong(self, N: int, K: int, W: int) -> float:
        dp, S = [1.0] + [0] * N, 0
        for i in range(1, N+1): # wrong: no longer throw if points >= K
            S += dp[i-1]
            if i >= W + 1:
                S -= dp[i-W-1]
            dp[i] = S / W
