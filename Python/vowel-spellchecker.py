...
class Solution(object):
    def spellchecker(self, wordlist, queries):
        ...
        """
        def devowel(word):
            # mask out all vowels. Not replace with '*' in case the word contains '*'
        ...
        for word in wordlist:
            wordlow = word.lower()
            caps.setdefault(wordlow, word)
            vows.setdefault(devowel(wordlow), word)
        ...
        for query in queries:
            ...
            lower = query.lower()
            if lower in caps:
                ans.append(caps[lower])
                continue

            devow = devowel(lower)
            if devow in vows:
                ans.append(vows[devow])
                continue
